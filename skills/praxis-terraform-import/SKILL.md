---
name: "praxis-terraform-import"
title: "Terraform State Import"
description: "Import existing cloud infrastructure into Facets-managed Terraform state. Guides through resource discovery, module preparation, incremental import block construction, drift reconciliation, and safe apply."
triggers: ["import", "terraform import", "import existing", "import infrastructure", "import cluster", "import resource", "state import", "import resources", "onboard infrastructure", "existing cloud", "bring into facets"]
version: "1.0"
category: "infrastructure"
tags: ["terraform", "import", "infrastructure", "gke", "gcp"]
icon: "📥"
---
...
If the import needs module changes, also load `Skill("build-facets-module")`.

## Immutable Safety Principles (Highest Priority)

1. **Never cause deletions or destructive actions** — plans must not show destroy or recreate operations
2. **Never cause disruptive changes** — no restarts, replacements, or service interruptions
3. **Minimize all changes** — only changes necessary for Facets compatibility
4. **Ensure future manageability** — the user must be able to modify resources via blueprint afterward
5. **Explicit consent required** — never apply without user approval after explaining the full plan

If any plan violates principles 1-3, **STOP** and investigate why before proceeding.
...
```bash
while true; do
  STATUS=$(raptor get releases -p <project> -e <environment> <release-id> -o json 2>/dev/null | grep -o '"status":"[^"]*"' | head -1)
  echo "$STATUS"
...
```bash
raptor logs release -p <project> -e <environment> <release-id> --stream > plan-logs.txt
```
...
```bash
# Encode locally (portable across GNU/Linux + BSD/macOS: read from stdin — BSD
# base64 takes no positional file and has no -w flag — then strip any wrapping)
base64 < /path/to/imports.tf | tr -d '\n'
...
### Log Analysis:
```bash
raptor logs release -p <project> -e <environment> <release-id> --stream > import-logs.txt
...
B64=$(base64 < /path/to/imports.tf | tr -d '\n')
raptor create custom-release -p <project> -e <environment> \
...
raptor get resources -p <project> <type>/<name> -o json > resource.json

# Edit to match actual AWS state (disable non-existent addons, remove phantom node groups, etc.)
# Then apply
raptor apply resource <type>/<flavor>/<version> -p <project> --spec-file resource.json
```
