...
Before jumping into individual profile optimization, perform a macro-level transitive DAG analysis of the entire workflow YAML using the `analyze_wf_dag.py` script. This reveals hot tables being scanned multiple times across sequential CTAS boundaries:
...
Once you have the text query profiles (plans) for your optimized run, parse them using `parse_query_profile.py` to verify the physical execution plan characteristics:
